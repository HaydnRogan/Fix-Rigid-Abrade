...
#ifdef FIX_CLASS
// clang-format off
FixStyle(rigid/shell,FixRigidShell);
// clang-format on
#else

#ifndef LMP_FIX_RIGID_SHELL_H
#define LMP_FIX_RIGID_SHELL_H

#include "fix.h"

namespace LAMMPS_NS {

class FixRigidShell : public Fix {
  friend class ComputeRigidLocal;

 public:
  FixRigidShell(class LAMMPS *, int, char **);
  ~FixRigidShell() override;
  ...
  double **vertexdata;   // array to store the normals, areas, and displacement velocities of each atom
 ...
 protected:
  ...
  int *angle2body;       // index of owned/ghost body this angle is in, -1 if not
                         // can point to original or any image of the body
