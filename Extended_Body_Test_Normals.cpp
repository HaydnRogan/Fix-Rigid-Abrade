    ...
    id_gravity(nullptr), onemols(nullptr)
{
  ...
  for (int i = 0; i < (atom->nlocal + atom->nghost); i++) {
    ...
    vertexdata[i][9] = vertexdata[i][10] = vertexdata[i][11] = 0.0;    // normals in global coordinates (x,y,z)
    vertexdata[i][12] = vertexdata[i][13] = vertexdata[i][14] = 0.0;    // vertex positions in global coordinates (x,y,z)
...
void FixRigidAbrade::compute_forces_and_torques()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    // referencing the torque from the vertex position placed at the outside edge of the atom
    offset = (1.0 + radius[i] / MathExtra::len3(dx));
    dx[0] *= offset;
    dx[1] *= offset;
    dx[2] *= offset;
...
void FixRigidAbrade::offset_vertices_inwards(){
  ...
  for (int i = 0; i < nlocal; i++){
    ...
    // offsetting atoms inwards by their radius along their normals  
    len_i = MathExtra::len3(displace[i]);
    displace[i][0] = displace[i][0] - vertexdata[i][0]*radius[i];
    displace[i][1] = displace[i][1] - vertexdata[i][1]*radius[i];
    displace[i][2] = displace[i][2] - vertexdata[i][2]*radius[i];
...
void FixRigidAbrade::offset_vertices_outwards(){
  ...
  for (int i = 0; i < nlocal; i++){
    ...
    // offsetting atoms outwards by their radius along their normals  
    displace[i][0] = displace[i][0] + vertexdata[i][0]*radius[i];
    displace[i][1] = displace[i][1] + vertexdata[i][1]*radius[i];
    displace[i][2] = displace[i][] + vertexdata[i][2]*radius[i];
...
void FixRigidAbrade::end_of_step()
{
  ...
  // setting abraded flag of remeshed bodies so that they can be resetup following the formal removal of remeshed atoms
    for (int ibody = 0; ibody < (nlocal_body + nghost_body); ibody++) {
  ...
  double yz = domain->yz;
    ...
    vertexdata[i][11] = global_normals[2];
      ...
      vertexdata[i][12] = x[i][0] + radius[i]*global_normals[0];
      vertexdata[i][13] = x[i][1] + radius[i]*global_normals[1];
      vertexdata[i][14] = x[i][2] + radius[i]*global_normals[2];  
  }
...
void FixRigidAbrade::resetup_bodies_static()
{
  ...
  for (ibody = 0; ibody < nlocal_body + nghost_body; ibody++) {
    ...
    // std::cout << me << ": resetting up body " << atom->tag[body[ibody].ilocal] << " at t = " << update->ntimestep << std::endl;
    xcm = body[ibody].xcm;
    xgc = body[ibody].xgc;
    xcm[0] = xcm[1] = xcm[2] = 0.0;
    xgc[0] = xgc[1] = xgc[2] = 0.0;
