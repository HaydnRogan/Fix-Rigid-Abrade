...
void FixRigidShell::initial_integrate(int vflag)
{
  ...
  for (int ibody = 0; ibody < nlocal_body; ibody++) {
    ...
    dtfm = dtf / b->mass;
    b->vcm[0] += dtfm * b->fcm[0] * stationary_flag;
    b->vcm[1] += dtfm * b->fcm[1] * stationary_flag;
    b->vcm[2] += dtfm * b->fcm[2] * stationary_flag;
    ...
    b->angmom[0] += dtf * b->torque[0] * stationary_flag;
    b->angmom[1] += dtf * b->torque[1] * stationary_flag;
    b->angmom[2] += dtf * b->torque[2] * stationary_flag;
...
void FixRigidShell::post_force(int /*vflag*/)
{
  ...
  // double **v = atom->v;  
  // std::cout << "v of 1678" << ": (" << v[1678][0] << ", " << v[1678][1] << ", " << v[1678][2] << ")" <<  std::endl;
 ...
 // update hardness due to variables
  ...
  // get local index of central atom ii
  int i = ilist[ii];
  ...
  // std::cout << "force acting on: " << i <<  std::endl;
  ...
  for (int jj = 0; jj < jnum; jj++){
    // get local index of neighbor to access its properties
    int j = jlist[jj];
    // std::cout << j << std::endl;
    ...
    // Check if the two atoms are approaching 

    

    if (gap_is_shrinking(i, j, v_rel, x_rel, &speed_of_approach)) {
  double **v = atom->v;

    // std::cout << "Gap is shrinking for " << i << " with vrel (" << v_rel[0] << ", " << v_rel[1] << ", " << v_rel[2] << ")" <<  std::endl;
    // std::cout << "v of " << i << ": (" << v[i][0] << ", " << v[i][1] << ", " << v[i][2] << ")" <<  std::endl;
    // std::cout << "v of " << j << ": (" << v[j][0] << ", " << v[j][1] << ", " << v[j][2] << ")" <<  std::endl;
    // std::cout << "j_radius:" << (atom->radius)[j] <<  std::endl;
    // std::cout << "j_mass:" << (atom->rmass)[j] <<  std::endl;

    // std::cout << " " <<  std::endl;
    // std::cout << "i: " << i <<  std::endl;
    // std::cout << "j_radius:" << (atom->radius)[j] <<  std::endl;
    // std::cout << "Speed of approach: " << speed_of_approach <<  std::endl;    
    
    // std::cout << "v_rel: (" << v_rel[0] << ", " << v_rel[1] << ", " << v_rel[2] << ")" <<  std::endl;
    // std::cout << "x_rel: (" << x_rel[0] << ", " << x_rel[1] << ", " << x_rel[2] << ")" <<  std::endl;
    // std::cout << " " <<  std::endl;


    // Calculate the displacement on atom i from an impact by j
    displacement_of_atom(i, (atom->radius)[j], speed_of_approach, v_rel, x_rel);

          }
        }
      // std::cout << "~ displacement_v of " << i << " (" << vertexdata[i][4] << ", " << vertexdata[i][5] << ", " << vertexdata[i][6] << ")" <<  std::endl;
      }
...
// std::cout << "x of " << i << ": (" << x[i][0] << ", " << x[i][1] << ", " << x[i][2] << ") " << std::endl;
...
bool FixRigidShell::gap_is_shrinking(int i, int neigh_index,
  ...
  x_rel[0] = x[neigh_index][0] - x[i][0];
  x_rel[1] = x[neigh_index][1] - x[i][1];
  x_rel[2] = x[neigh_index][2] - x[i][2];
  ...
  *vnorm_p = MathExtra::dot3(v_rel, normal);
  ...
  // std::cout << "Gap of " << i << " and " << neigh_index << " : " << *vnorm_p << std::endl;
  // std::cout << "v of " << i << ": (" << v[i][0] << ", " << v[i][1] << ", " << v[i][2] << ") " << std::endl;
  // std::cout << "v of " << neigh_index << ": (" << v[neigh_index][0] << ", " << v[neigh_index][1] << ", " << v[neigh_index][2] << ") " << std::endl;
  // std::cout << "x of " << i << ": (" << x[i][0] << ", " << x[i][1] << ", " << x[i][2] << ") " << std::endl;
  // std::cout << "normal of " << i << ": (" << vertexdata[i][0] << ", " << vertexdata[i][1] << ", " << vertexdata[i][2] << ") " << std::endl;
  // std::cout << " " << std::endl;
  return *vnorm_p < 0;
}
...
void FixRigidShell::final_integrate()
{
  ...
  set_v();
    ...
    // Need to convert the displacement velocity into the reference frame of the the displace position
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    MathExtra::transpose_matvec(b->ex_space,b->ey_space,b->ez_space,
                                global_displace_vel,body_displace_vel);
    ...
    std::cout << "body_displace: (" << body_displace_vel[0] << ", " << body_displace_vel[1] << ", " << body_displace_vel[2] << ") " << std::endl;
